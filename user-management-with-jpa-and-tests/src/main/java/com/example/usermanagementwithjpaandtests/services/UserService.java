...
public class UserService implements UserDetailsService {
    ...
    public UserResponse createUser(UserRequest userRequest) {
        ...
        if (userRequest.getRoles() != null) {
            roles = userRequest.getRoles().stream().map(it -> roleRepository.findById(it).orElse(null)).filter(Objects::nonNull).collect(Collectors.toSet());
        } else {
    ...
    public UserResponse editUser(Long id, UserRequest userRequest) {
        ...
        if (user != null) {
            ...
            if(userRequest.getRoles() != null) {
                Set<Role> roles = userRequest.getRoles().stream().map(it -> roleRepository.findById(it).orElse(null)).filter(Objects::nonNull).collect(Collectors.toSet());
                user.setRoles(roles);
            }
            userRepository.save(user);
            response = mapper.map(user, UserResponse.class);
    ...
    public boolean hasAdminRole(UserRequest userRequest) {
        ...
        if(userRequest.getRoles() != null) {
            ...
            has = (adminRole != null && userRequest.getRoles().stream().anyMatch(it -> Objects.equals(adminRole.getId(), it)));
        }
        return has;
    }
